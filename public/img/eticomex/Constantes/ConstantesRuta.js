// ETICOMEX
const PRECIO1_TECNO = 0.8;
const PRECIO2_TECNO = 0.78;
const PRECIO3_TECNO = 0.76;
const PROYECTO = `/eticomex`; // constante para carpeta de imagenes de la empresa
const EMPRESA = `ETICOMEX`; // constante nombre empresa
const FAC_SAS = `ETICOMEX`; // constante factura empresa
const FAC_COL = `N/A`; // constante factura empresa
const FAC_ESP = `ETICOMEX ESPECIAL`; // constante factura empresa
const CORREO_TALENTO = `diego.wilches@acobarras.com`; // constante correo de talento humano
const ID_CLI_PROV_PQR = 1931;
const ID_CLI_PROV_NO_PRODUCIR = 2441;
const FAC_ELECTRONICA = 0; // 1 para realizar el ingreso de portafolio con lector; 0 ingreso manual
const IMPRESION_API = 0;
const SERVIDOR_IMPRESION = 'https://192.168.2.41:3000';// Este es un servidor local que se configura para gestionar las impresion 
const DOS_MODULOS = false; // Se activa cuando el alistamiento se divide en los dos modulos de almacen y produccion